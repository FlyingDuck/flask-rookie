# -*- coding: utf-8 -*-
from flask import flash, redirect, url_for, render_template, request, jsonify, abort, make_response
...
from app.task.mail import send_async_email
        ...
        if form.validate_on_submit():
            ...
            if passwd != rp_passwd:
                ...
                if id is not None:
                    ...
                    send_email(email, u'欢迎注册pkyx,请确认你的账户', 'email', token=token)
                    return redirect(url_for('main.index'))
                flash(u'注册失败', 'WARNING')
        else:
        ...
        if form.validate_on_submit():
            ...
            if db_user is not None:
                ...
                if User.verify_passwd(db_passwd, form.password.data):
                    ...
                    login_user(user)
                    return jsonify(status=True, reason=u"登录成功", redirect_url=url_for('main.index'))
                else:
                    return jsonify(status=False, reason=u"邮箱或密码错误")
            else:
                return jsonify(status=False, reason=u"不存在该用户")
        return jsonify(status=False, reason=u"登录失败")
...
def profile_edit():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            flash(u'资料修改失败', 'red')
    return render_template('profile_edit.html', user=user, form=form, title=u'编辑资料')
